...
void ibuffer_stage (VX_decode decode,VX_decode *ibuffer,bool write_ibuffer,bool read_ibuffer)
{
	...
	if((deq_wid==0 && valid_table[0]==1 &&valid_table[1]==0 &&valid_table[2]==0 &&valid_table[3]==0)
		||(deq_wid==1 &&valid_table[0]==1 &&valid_table[2]==0 &&valid_table[3]==0)
		||(deq_wid==2 &&valid_table[0]==1  &&valid_table[3]==0)
		||(deq_wid==3 &&valid_table[0]==1  )) deq_wid=0;
	else if((deq_wid==1 && valid_table[0]==0 &&valid_table[1]==1 &&valid_table[2]==0 &&valid_table[3]==0)
		||(deq_wid==2 &&valid_table[0]==0 &&valid_table[1]==1 &&valid_table[3]==0)
		||(deq_wid==3 &&valid_table[0]==0  &&valid_table[1]==1)
		||(deq_wid==0 &&valid_table[1]==1  )) deq_wid=1;
	else if((deq_wid==2 && valid_table[0]==0 &&valid_table[1]==0 &&valid_table[2]==1 &&valid_table[3]==0)
		||(deq_wid==3 &&valid_table[2]==1 &&valid_table[1]==0 &&valid_table[0]==0)
		||(deq_wid==0 &&valid_table[2]==1  &&valid_table[1]==0)
		||(deq_wid==1 &&valid_table[2]==1  )) deq_wid=2;
	else deq_wid=3;
    ...
    for ( int i = 0; i <NUM_WARPS; i++)
	{
		...
		if(reading[i])
		{
			if (added_warps >= 1)
			{
				assign_ibuffer(buffer[deq_wid][IBUF_SIZE-(used_r[deq_wid]+1)], &*ibuffer);
			}
    ...
    warp_removed = read_ibuffer && !(write_ibuffer && decode.wid == deq_wid) && !not_alm_empty_r[deq_wid];
    ...
    for ( int i = 0; i <NUM_WARPS; i++)
    {
		...
		{
		 full_r[i] = 0;
		 if (used_r[i] == 1) not_empty_r[i] = 0;
		 if (used_r[i] == 2)  not_alm_empty_r[i] = 0;
		}
        if(!full_r[i])used_r[i] = used_r[i] + writing[i] - reading[i];
	}
